...
func Test_Lexer_Next_SkipsTrivia(t *testing.T) {
	...
	for tcName, tc := range map[string]struct {
		...
		"When source starts with a space, the space is skipped.": {
			inSource: " scope",
			want: []token.Token{
				makeToken(token.TokenScope, "scope", 1, 6),
		...
		"When source starts with a line comment, the comment is skipped.": {
			inSource: "// comment\nscope",
			want: []token.Token{
				makeToken(token.TokenScope, "scope", 11, 16),
...
func Test_Lexer_Next_ScansString(t *testing.T) {
	...
	for tcName, tc := range map[string]struct {
		...
		"When lexing a string literal with plain text, a string token is returned.": {
			inSource: `"abc"`,
			want: []token.Token{
				makeToken(token.TokenString, `"abc"`, 0, 5),
		...
		"When lexing a string literal with an escaped carriage return, a string token is returned.": {
			inSource: `"\r"`,
			want: []token.Token{
				makeToken(token.TokenString, `"\r"`, 0, 4),
				makeToken(token.TokenEOF, "", 4, 4),
			},
		},
		...
		"When lexing a string literal with an invalid escape, an invalid token is returned.": {
			...
			want: []token.Token{
				...
				makeToken(token.TokenIdentifier, "x", 2, 3),
		...
		"When lexing a string literal with a newline, an invalid token is returned.": {
			inSource: "\"abc\n\"",
			want: []token.Token{
				makeToken(token.TokenInvalid, "\"abc", 0, 4),
...
func Test_Lexer_Next_ScansCharacter(t *testing.T) {
	...
	for tcName, tc := range map[string]struct {
		...
		"When lexing a character literal with a plain character, a character token is returned.": {
			inSource: `'a'`,
			want: []token.Token{
				makeToken(token.TokenCharacter, `'a'`, 0, 3),
		...
		"When lexing a character literal with an escaped newline, a character token is returned.": {
			inSource: `'\n'`,
			want: []token.Token{
				makeToken(token.TokenCharacter, `'\n'`, 0, 4),
				makeToken(token.TokenEOF, "", 4, 4),
			},
		},
		"When lexing a character literal with an escaped carriage return, a character token is returned.": {
			inSource: `'\r'`,
			want: []token.Token{
				makeToken(token.TokenCharacter, `'\r'`, 0, 4),
				makeToken(token.TokenEOF, "", 4, 4),
			},
		},
		...
		"When lexing a character literal with a newline, an invalid token is returned.": {
			inSource: "'a\n'",
			want: []token.Token{
				makeToken(token.TokenInvalid, "'a", 0, 2),
				makeToken(token.TokenInvalid, `'`, 3, 4),
				makeToken(token.TokenEOF, "", 4, 4),
			},
		},
...
func Test_Lexer_Next_ScansIdentifiers(t *testing.T) {
	...
	for tcName, tc := range map[string]struct {
		...
		"When lexing the scope keyword, a scope token is returned.": {
			inSource: "scope",
			want: []token.Token{
				makeToken(token.TokenScope, "scope", 0, 5),
				makeToken(token.TokenEOF, "", 5, 5),
			},
		},
		"When lexing the include keyword, an include token is returned.": {
			inSource: "include",
			want: []token.Token{
				makeToken(token.TokenInclude, "include", 0, 7),
		...
		"When lexing the exclude keyword, an exclude token is returned.": {
			inSource: "exclude",
			want: []token.Token{
				makeToken(token.TokenExclude, "exclude", 0, 7),
				makeToken(token.TokenEOF, "", 7, 7),
			},
		},
		"When lexing the definitions keyword, a definitions token is returned.": {
			...
			want: []token.Token{
				makeToken(token.TokenDefinitions, "definitions", 0, 11),
		...
		"When lexing the tokens keyword, a tokens token is returned.": {
			inSource: "tokens",
			want: []token.Token{
				makeToken(token.TokenTokens, "tokens", 0, 6),
				makeToken(token.TokenEOF, "", 6, 6),
			},
		},
		"When lexing the skip keyword, a skip token is returned.": {
			inSource: "skip",
			want: []token.Token{
				makeToken(token.TokenSkip, "skip", 0, 4),
				makeToken(token.TokenEOF, "", 4, 4),
			},
		},
		"When lexing the rules keyword, a rules token is returned.": {
			inSource: "rules",
			want: []token.Token{
				makeToken(token.TokenRules, "rules", 0, 5),
				makeToken(token.TokenEOF, "", 5, 5),
			},
		},
		"When lexing the rule keyword, a rule token is returned.": {
			inSource: "rule",
			want: []token.Token{
				makeToken(token.TokenRule, "rule", 0, 4),
				makeToken(token.TokenEOF, "", 4, 4),
			},
		},
		"When lexing the match keyword, a match token is returned.": {
			inSource: "match",
			want: []token.Token{
				makeToken(token.TokenMatch, "match", 0, 5),
				makeToken(token.TokenEOF, "", 5, 5),
			},
		},
		"When lexing the where keyword, a where token is returned.": {
			inSource: "where",
			want: []token.Token{
				makeToken(token.TokenWhere, "where", 0, 5),
				makeToken(token.TokenEOF, "", 5, 5),
			},
		},
		"When lexing the report keyword, a report token is returned.": {
			inSource: "report",
			want: []token.Token{
				makeToken(token.TokenReport, "report", 0, 6),
				makeToken(token.TokenEOF, "", 6, 6),
			},
		},
		"When lexing the info keyword, an info token is returned.": {
			inSource: "info",
			want: []token.Token{
				makeToken(token.TokenInfo, "info", 0, 4),
				makeToken(token.TokenEOF, "", 4, 4),
			},
		},
		"When lexing the warn keyword, a warn token is returned.": {
			inSource: "warn",
			want: []token.Token{
				makeToken(token.TokenWarn, "warn", 0, 4),
				makeToken(token.TokenEOF, "", 4, 4),
			},
		},
		"When lexing the err keyword, an err token is returned.": {
			inSource: "err",
			want: []token.Token{
				makeToken(token.TokenErr, "err", 0, 3),
				makeToken(token.TokenEOF, "", 3, 3),
			},
		},
		"When lexing the at keyword, an at token is returned.": {
			inSource: "at",
			want: []token.Token{
				makeToken(token.TokenAt, "at", 0, 2),
				makeToken(token.TokenEOF, "", 2, 2),
			},
		},
		"When lexing a lowercase identifier, an identifier token is returned.": {
			inSource: "letter",
			want: []token.Token{
				makeToken(token.TokenIdentifier, "letter", 0, 6),
				makeToken(token.TokenEOF, "", 6, 6),
			},
		},
		"When lexing an uppercase identifier, an identifier token is returned.": {
			inSource: "Identifier",
			want: []token.Token{
				makeToken(token.TokenIdentifier, "Identifier", 0, 10),
				makeToken(token.TokenEOF, "", 10, 10),
			},
		},
		"When lexing an identifier with digits, an identifier token is returned.": {
			inSource: "letter1",
			want: []token.Token{
				makeToken(token.TokenIdentifier, "letter1", 0, 7),
				makeToken(token.TokenEOF, "", 7, 7),
			},
		},
		"When lexing an identifier with underscores, an identifier token is returned.": {
			...
			want: []token.Token{
				makeToken(token.TokenIdentifier, "identifier_part", 0, 15),
		...
		"When lexing an identifier that starts with a keyword, an identifier token is returned.": {
			inSource: "scopex",
			want: []token.Token{
				makeToken(token.TokenIdentifier, "scopex", 0, 6),
				makeToken(token.TokenEOF, "", 6, 6),
			},
		},
	} {
...
func Test_Lexer_Next_ScansIntegers(t *testing.T) {
	...
	for tcName, tc := range map[string]struct {
		...
		"When lexing a single digit, an integer token is returned.": {
			inSource: "1",
			want: []token.Token{
				makeToken(token.TokenInteger, "1", 0, 1),
				makeToken(token.TokenEOF, "", 1, 1),
			},
		},
		"When lexing multiple digits, an integer token is returned.": {
			inSource: "123",
			want: []token.Token{
				makeToken(token.TokenInteger, "123", 0, 3),
				makeToken(token.TokenEOF, "", 3, 3),
			},
		},
	} {
...
func Test_Lexer_Next_PreservesSpans(t *testing.T) {
	...
	want := []token.Token{
		...
		makeToken(token.TokenInclude, "include", 8, 15),
		makeToken(token.TokenString, "\"**/*.go\"", 16, 25),
		makeToken(token.TokenExclude, "exclude", 26, 33),
		makeToken(token.TokenString, "\"vendor/**\"", 34, 45),
		makeToken(token.TokenRightBrace, "}", 46, 47),
		makeToken(token.TokenDefinitions, "definitions", 48, 59),
		makeToken(token.TokenLeftBrace, "{", 60, 61),
		makeToken(token.TokenIdentifier, "letter", 62, 68),
		makeToken(token.TokenEqual, "=", 69, 70),
		makeToken(token.TokenCharacter, "'a'", 71, 74),
		makeToken(token.TokenDotDot, "..", 74, 76),
		makeToken(token.TokenCharacter, "'z'", 76, 79),
		makeToken(token.TokenPipe, "|", 80, 81),
		makeToken(token.TokenCharacter, "'A'", 82, 85),
		makeToken(token.TokenDotDot, "..", 85, 87),
		makeToken(token.TokenCharacter, "'Z'", 87, 90),
		makeToken(token.TokenRightBrace, "}", 91, 92),
		makeToken(token.TokenTokens, "tokens", 93, 99),
		makeToken(token.TokenLeftBrace, "{", 100, 101),
		makeToken(token.TokenIdentifier, "Whitespace", 102, 112),
		makeToken(token.TokenEqual, "=", 113, 114),
		makeToken(token.TokenCharacter, "' '", 115, 118),
		makeToken(token.TokenPlus, "+", 118, 119),
		makeToken(token.TokenSkip, "skip", 120, 124),
		makeToken(token.TokenRightBrace, "}", 125, 126),
		makeToken(token.TokenRules, "rules", 127, 132),
		makeToken(token.TokenLeftBrace, "{", 133, 134),
		makeToken(token.TokenRule, "rule", 135, 139),
		makeToken(token.TokenIdentifier, "PublicIdentifier", 140, 156),
		makeToken(token.TokenLeftBrace, "{", 157, 158),
		makeToken(token.TokenMatch, "match", 159, 164),
		makeToken(token.TokenIdentifier, "Identifier", 165, 175),
		makeToken(token.TokenWhere, "where", 176, 181),
		makeToken(token.TokenIdentifier, "Identifier", 182, 192),
		makeToken(token.TokenDot, ".", 192, 193),
		makeToken(token.TokenIdentifier, "length", 193, 199),
		makeToken(token.TokenGreater, ">", 200, 201),
		makeToken(token.TokenInteger, "1", 202, 203),
		makeToken(token.TokenReport, "report", 204, 210),
		makeToken(token.TokenWarn, "warn", 211, 215),
		makeToken(token.TokenAt, "at", 216, 218),
		makeToken(token.TokenIdentifier, "Identifier", 219, 229),
		makeToken(token.TokenString, "\"Public identifier found\"", 230, 255),
...
func makeToken(kind token.TokenKind, text string, start, end location.Position) token.Token {
	return token.Token{
		Kind: kind,
		Text: text,
		Span: location.Span{
